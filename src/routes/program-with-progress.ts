...
function sendProgress(jobId: string, data: any) {
  ...
  if (stream && !stream.destroyed) {
    try {
      ...
      console.log(`Sending progress to ${jobId}:`, data.type, data.message || data.status);
      stream.write(message);
    } catch (error) {
      console.error(`Error sending progress to ${jobId}:`, error);
      progressStreams.delete(jobId);
    }
  }
}
...
async function processProgram(jobId: string, requestBody: any) {
  try {
    ...
    sendProgress(jobId, {
      ...
      name: 'Sourcing Agent',
      status: 'starting',
      message: '📚 Creating sourcing plan and reference materials...'
    });
    ...
    sendProgress(jobId, {
      ...
      name: 'Sourcing Agent',
      status: 'completed',
      message: `✅ Created sourcing plan with ${sources.sources.length} sources`,
      data: { sourceCount: sources.sources.length }
    });

    // Steps 4-8: Content Generation Pipeline
    sendProgress(jobId, {
      type: 'step',
      step: 4,
      name: 'Content Generation Pipeline',
      status: 'starting',
      message: `🎯 Starting content generation for ${matrix.sessions.length} sessions...`
    });
    ...
    for (let i = 0; i < matrix.sessions.length; i++) {
      ...
      sendProgress(jobId, {
        type: 'session',
        sessionIndex: i + 1,
        sessionTotal: matrix.sessions.length,
        sessionTitle: s.title,
        status: 'starting',
        message: `📝 Processing session ${i + 1}/${matrix.sessions.length}: ${s.title}`
      });

      try {
        // Step 4: Article Agent
        sendProgress(jobId, { type: 'session_substep', step: 'article', status: 'generating', message: '📝 Generating article...' });
        console.log("  📝 Generating article...");
        const articleInput = { program: normalized.program, session: s, sources: sources.sources };
        const article = await prose(renderPrompt(ARTICLE_PROMPT, articleInput), articleInput);

        // Step 5: Video Script Agent
        sendProgress(jobId, { type: 'session_substep', step: 'video', status: 'generating', message: '🎥 Generating video script...' });
        ...
        // Step 6: Quiz Agent
        sendProgress(jobId, { type: 'session_substep', step: 'quiz', status: 'generating', message: '❓ Generating quiz...' });
        console.log("  ❓ Generating quiz...");
        const quizInput = { program: normalized.program, session: s };
        const quiz = await chat(renderPrompt(QUIZ_PROMPT, quizInput), quizInput);

        // Step 7: Exercise Agent
        sendProgress(jobId, { type: 'session_substep', step: 'exercise', status: 'generating', message: '💬 Generating exercise...' });
        ...
        sendProgress(jobId, { type: 'session_substep', step: 'qc', status: 'processing', message: '✅ Running QC and fixes...' });
        ...
        sendProgress(jobId, {
          ...
          sessionTitle: s.title,
          status: 'completed',
          message: `✅ Completed ${s.title} (QC Score: ${record.qcScore}/100)`,
          qcScore: record.qcScore
        });
        ...
        sendProgress(jobId, {
          ...
          sessionTitle: s.title,
          status: 'error',
          message: `❌ Error processing session ${s.title}`,
          error: (sessionError as Error).message
        });
      }
    }
    ...
    sendProgress(jobId, {
      ...
      name: 'Packager Agent',
      status: 'starting',
      message: '📦 Packaging final program manifest...'
    });
